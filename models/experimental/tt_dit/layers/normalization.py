...
class GroupNorm(Module):
    ...
    def __init__(
        ...
        *,
        ...
        torch_ref: torch.nn.GroupNorm | None = None,
    ) -> None:
        """
        Args:
        ...
        num_channels = num_channels or torch_ref.num_channels
        num_groups = num_groups or torch_ref.num_groups
        eps = eps or torch_ref.eps
        core_grid = core_grid or ttnn.CoreGrid(x=8, y=8)  # mesh_device.core_grid # Issue on 6U 8x9 grid

        num_devices = tuple(mesh_device.shape)[mesh_axis] if mesh_axis is not None else 1

        assert num_channels % num_groups == 0, "num_channels must be divisible by num_groups"
        assert num_groups % num_devices == 0, "num_groups must be divisible by num_devices"

        num_local_channels = num_channels // num_devices
        ...
        num_padded_groups = num_groups // num_devices * num_padded_channels // num_local_channels

        num_virtual_cols = ttnn.operations.normalization.dram_group_norm_virtual_columns(
        ...
        weight_shape = [
            num_devices,
            1,
            math.ceil(num_padded_channels // num_virtual_cols / 32) * num_virtual_cols,
            32,
        ]
        ...
        self.num_virtual_cols = num_virtual_cols
