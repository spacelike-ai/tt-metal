...
class MistralRmsNorm(Module):
    def __init__(self, size: int, *, eps: float, ctx: MistralContext) -> None:
        ...
        self._use_rms_workaround = size % (tp_axis_size * 32) != 0

        self.weight = Parameter(total_shape=[size], mesh_axes=[ctx.tp_axis], device=ctx.device)
...
def create_rope_tensors(
    ...
    if attention_mask is not None:
        ...
        position_ids = torch.arange(sequence_length).view(1, 1, -1).expand(1, batch_size, -1)

    inv_freq = rope_theta ** (-torch.arange(0, head_dim, 2, dtype=torch.int64).to(dtype=torch.float) / head_dim)
    inv_freq_expanded = inv_freq[None, None, :, None].float().expand(1, batch_size, -1, 1)
