...
def test_attention(
    ...
    torch_spatial, torch_prompt = torch_model(
        spatial=spatial_input, prompt=prompt_input, image_rotary_emb=(rope_cos, rope_sin)
    )
    ...
    tt_spatial_rope_cos = bf16_tensor(
        rope_cos[prompt_seq_len:], device=mesh_device, mesh_axis=sp_axis, shard_dim=-2
    )  # TODO: float32?
    tt_spatial_rope_sin = bf16_tensor(
        rope_sin[prompt_seq_len:], device=mesh_device, mesh_axis=sp_axis, shard_dim=-2
    )  # TODO: float32?
    tt_prompt_rope_cos = (
        bf16_tensor(rope_cos[:prompt_seq_len], device=mesh_device) if joint_attention else None
    )  # TODO: float32?
    tt_prompt_rope_sin = (
        bf16_tensor(rope_sin[:prompt_seq_len], device=mesh_device) if joint_attention else None
    )  # TODO: float32?
