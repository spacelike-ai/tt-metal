...
from .normalization import (
    TtAdaLayerNormContinuous,
    TtAdaLayerNormParameters,
    TtAdaLayerNormZero,
    TtLayerNormParameters,
    TtSD35AdaLayerNormZeroX,
)
...
class TtJointTransformerBlockParameters:
    ...
    attn2: TtAttentionParameters | None
    norm1: TtAdaLayerNormParameters
    norm2: TtLayerNormParameters
    norm1_context: TtAdaLayerNormParameters | None
    norm2_context: TtLayerNormParameters | None
    ff: TtFeedForwardParameters
    ff_context: TtFeedForwardParameters | None
...
class TtJointTransformerBlock:
    def __init__(
        ...
        *,
        ...
        self.ff = TtFeedForward(dim=dim, dim_out=dim, approximate="tanh")

        if context_pre_only:
            self.norm1_context = TtAdaLayerNormContinuous(dim, dim)
            self.norm2_context = None
            self.ff_context = None
        else:
            self.norm1_context = TtAdaLayerNormZero(dim)
            self.norm2_context = torch.nn.LayerNorm(dim, elementwise_affine=False, eps=1e-6)
            self.ff_context = TtFeedForward(dim=dim, dim_out=dim, approximate="tanh")

        if use_dual_attention:
            self.norm1 = TtSD35AdaLayerNormZeroX(dim)
