...
grid_1d_configs: dict[tuple[int, int, int, int, int], tuple[int, int, int]] = {
    ...
    (32, 1216, 2560, 11, 10): (2, 3, 3),  # 27.4 μs -> 20.6 μs
    # Mistral3 tp4 decode qkv and gate/up — WH 8×8 grid (2026-09-12)
    (32, 5120, 1536, 8, 8): (8, 1, 1),  # 59.2 μs -> 53.1 μs
    (32, 5120, 16384, 8, 8): (1, 8, 4),  # 452.9 μs -> 436.0 μs
}
...
#
# ``M=-1`` is a wildcard: ``get_fabric_agmm_config`` tries an exact ``(M, K, N, chunks)`` match
# first, then ``(-1, K, N, chunks)``. LTX entries use the wildcard because in-model M varies with
# resolution and SP padding. Flux entries are exact-M; an exact-match miss is harmless (the caller
# falls back to ``all_gather_minimal_matmul_async``).
#
# Every entry must be measured on device before it lands here: a guessed entry silently ships an
...
fabric_agmm_configs: dict[ttnn.CoreCoord, dict[tuple, FabricAGMMConfig]] = {
    ttnn.CoreCoord(12, 10): {
        ...
        # LTX: M-independent (wildcard). Same blocking as the previous (K, N, chunks) keys.
        (-1, 4096, 1024, 1): FabricAGMMConfig(ttnn.CoreCoord(12, 8), (0, 8), 16, 8, 4, 2, 2, 3, 8),
        # video to_gate_logits: per-device N = num_heads/tp = 8, tile-padded to 32 (1 tile)
        (-1, 4096, 32, 1): FabricAGMMConfig(ttnn.CoreCoord(12, 8), (0, 8), 16, 8, 1, 2, 1, 3, 8),
        # audio_to_video_attn.to_q (a2v cross): K = video_dim = 4096, N = audio_dim/tp = 512
        (-1, 4096, 512, 1): FabricAGMMConfig(ttnn.CoreCoord(12, 8), (0, 8), 16, 8, 2, 2, 2, 3, 8),
        # -----------------------------------------------------------------------------------------
...
def get_fabric_agmm_config(M, K, N, chunks, device_core_grid) -> FabricAGMMConfig | None:
    ...
    ``all_gather_minimal_matmul_async`` path. Keyed on ``(M, K, N, chunks)``, where ``N`` is the
    per-device weight width (the packed [gate|up] width for fused SwiGLU). ``M=-1`` in the table
    is a wildcard: an exact match is tried first, then ``(-1, K, N, chunks)``.
    ...
    if os.environ.get("DISABLE_FABRIC_AGMM") in ("1", "true", "True"):
        return None
    table = fabric_agmm_configs.get(device_core_grid, {})
    cfg = table.get((M, K, N, chunks))
    if cfg is None and M != -1:
        cfg = table.get((-1, K, N, chunks))
    return cfg
...
def register_fabric_agmm_configs(configs: dict) -> None:
    ...
    Args:
        configs: Mapping from ``ttnn.CoreCoord`` (device core-grid) to dict of
            ``(M, K, N, chunks)`` -> :class:`FabricAGMMConfig`. Use ``M=-1`` for an
            M-independent wildcard entry.
